...
const TrackDetails = ({ trackId }) => {
  ...
  useEffect(() => {
    ...
    const fetchTrack = async () => {
      try {
        const data = await fetchData("track", trackId, () =>
          getTrackById(localStorage.getItem("access_token"), trackId)
        );
        setTrack(data);
      } catch (error) {
    ...
    fetchTrack();
  }, [trackId, fetchData, track]);

  if (isLoading) {
    return <p>Cargando track...</p>;
  }

  if (!track) {
    return <p>No se encontró el track.</p>;
  }
